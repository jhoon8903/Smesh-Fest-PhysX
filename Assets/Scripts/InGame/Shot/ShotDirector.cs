...
namespace InGame.Shot
{
    public sealed class ShotDirector : IDisposable
    {
        ...
        private readonly WorldObjectControllerRegistry _controllers;
        private readonly CannonController _cannon;
        ...
        public ShotDirector(PoolFactory poolFactory, PoolConfig ballPool, CannonView cannon,
            ...
            WorldObjectControllerRegistry controllers)
        {
            ...
            _cannon = _controllers.GetOrCreate(cannon != null ? cannon : throw new ArgumentNullException(nameof(cannon)));

            _ballParent = ballParent;
            _activeShots = new ActiveShot[Mathf.Max(1, ballPool.MaxPool)];
        }

        public int ActiveShotCount { get; private set; }

        public bool TryFire(Vector3 target)
        {
            if (_disposed || !IsFinite(target) || !IsFinite(_ballSettings.GravityActivationWorldZ) || ActiveShotCount >= _activeShots.Length) return false;
            if (!_cannon.TryAim(target - _cannon.MuzzlePosition)) return false;
            Vector3 gravity = _physXSettings.Gravity;
            Physics.gravity = gravity;
            Vector3 origin = _cannon.MuzzlePosition;
            if (!BallLaunchVelocity.TryCalculate(origin, target, _ballSettings.TrajectoryMode, _ballSettings.LaunchSpeed, _ballSettings.CurveFlightSeconds, gravity, out Vector3 velocity)) return false;
            ...
            if (ball == null || !_controllers.TryGet(ball, out BallController controller))
            {
                lease.Return();
                return false;
            }

            uint epoch = controller.RentalEpoch;
            if (!controller.TryLaunch(epoch, velocity, _ballSettings.TrajectoryMode))
            {
                lease.Return();
                return false;
            }

            GroundFadeReturn fade = controller.GroundFadeReturn;
            if (fade == null)
            {
                lease.Return();
                return false;
            }
            _activeShots[ActiveShotCount++] = new ActiveShot { Controller = controller, Fade = fade, Epoch = epoch };
            return true;
        }
        ...
        public void Tick(float deltaTime)
        {
            ...
            for (int i = ActiveShotCount - 1; i >= 0; i--)
            {
                ActiveShot shot = _activeShots[i];
                BallController controller = shot.Controller;
                if (controller == null || !controller.IsCurrentRental(shot.Epoch))
                {
                    RemoveAt(i);
                    continue;
                }
                if (shot.Fade != null && shot.Fade.IsFading) continue;
                shot.Elapsed += deltaTime;
                _activeShots[i] = shot;
                if (shot.Elapsed < _ballSettings.MaxLifetimeSeconds && controller.Position.y >= _ballSettings.RecycleBelowY) continue;
        ...
        public void TickPhysics()
        {
            ...
            {
                ActiveShot shot = _activeShots[i];
                shot.Controller?.TickPhysics(shot.Epoch, gravityActivationWorldZ);
            }
        }
